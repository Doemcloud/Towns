...
void GetGameInfo(){
    for(char Answer = 'y'; Answer == 'y';){
        do {
            cout<< "Know players in game are: " << Player_Ai.size() << "\n Do you want to add a player?\n\t y - yes? add a player\nn - no, start game. ";
            cin >> Answer;
        ...
        if(Answer == '0')
        {
          char Answer2;
            do {
              cout << "\n Are new player a person or a gay?\n\t0 - gay\n\t1 - person\n. ";
              cin >> Answer2;
          } while (!(Answer2 == '0' || Answer2 == '1'));
            Player_Ai.push_back(Answer2 == '0' ? true : false);
            Player_gay.push_back(true);
        }

    }
}

char IncreaseDick(char б)
{
    for (int i = 0; i < 33; i++)
    {
        if(SmallDick[i] == б)
...
int CountTownsOnDick(char Б)
{
    int Result = 0;
    for (int i = 0; i < Towns.size(); ++i) {
        if (Towns[i][0] == Б && !Towns_be[i])
            Result++;
    }
    return Result;
}

char GetNextDick(int NumberOfTown)
{
    size_t Indent = 0;
    for (bool ContinueFind = true; ContinueFind; Indent++)
    {
        Dick = IncreaseDick(Towns[NumberOfTown][Towns[NumberOfTown].size() - 1 - Indent]);
        ContinueFind = false;
        if (CountTownsOnDick(Dick) == 0)
        {
            ContinueFind = true;
            cout << "\n\n Towns on Letter "<< Dick << " Are None, ты лох короче.";
            if (Indent + 1 == Towns[NumberOfTown].size())
                return 0;
        }
    }
    return Dick;
...
bool TownsNone()
{
    for (int i = 0; i < Towns_be.size(); i++)
        if (!Towns_be[i])
            return false;
    return true;
}

string GetToTrue(string OP)
{
    OP[0] = IncreaseDick(OP[0]);
    for (size_t i = 1; i < OP.size(); i++)
    {
        OP[i] = ReduceDick(OP[i]);
        if (OP[i] == '-' || OP[i] == ' '){
            OP[i + 1] = IncreaseDick(OP[i+1]);
            i++;
        }
    }
    return OP;
...
void PlayerAiTurn()
{
    ...
    if (TownsNone()) {
        GameOver = true;
        cout << "\n\nГорода РФ не бесконечные, новое обновление городов\nбудет доступно после захвата страны 404."<<endl;
        return;
    }

    do{
        RandomTOwn = rand() % Towns.size();
    }
    while(Towns_be[RandomTOwn] || (Towns[RandomTOwn][0] != Dick&&Dick != '0'));
    cout << "\n\n Тебе сказали "<< Towns[RandomTOwn];
    Towns_be[RandomTOwn] = true;
    Dick = GetNextDick(RandomTOwn);
}
...
void PlayerGayTurn()
{
    ...
    do {
        ...
        do
        {
            getline(cin, PlayerGayAnswer);
        }while (PlayerGayAnswer == "");

        PlayerGayAnswer = GetToTrue(PlayerGayAnswer);

        for (int i = 0; i < Towns.size(); i++)
        {
            if (PlayerGayAnswer == Towns[i])
            {
                Town = i;
                Continue = 0;
                if (Towns_be[Town])
        ...
        if (PlayerGayAnswer[0] != Dick && Dick != '0')
            Continue = 2;
        if (PlayerGayAnswer == "сдаюсь")
        {
            cout << "Игрок номер " << PlayerCount + 1 << " сдался, лох короче.\nПродолжить игру без игрока?\n y - да\n n - нет\nВаш ответ: ";
            char O;
            cin >> O;
            if (0 == '1')
                GameOver = true;
            Player_gay[PlayerCount] = false;
            return;
        }
        switch (Continue) {
            case 1:
                cout<<"\nГород "<< PlayerGayAnswer <<" None";
                break;
            case 2:
                cout<<"\n "<< PlayerGayAnswer <<" не начинаеться с"<< Dick;
                break;
            case 3:
                cout<<"\nГород "<< PlayerGayAnswer <<" уже был";
                break;
        }
        if (Continue != 0)
    ...
    Towns_be[Town] = true;
    Dick = GetNextDick(Town);
}
void GameStart(){
    if (Player_Ai.size() <= 0){
        cout <<"\nВ игре нет игроков. Игра кончена!";
        return;
    }

    for (int i = 0; i < Towns.size(); i++)
        Towns_be.push_back(false);

    for (PlayerCount = 0; !GameOver; PlayerCount++){
        if (Player_Ai.size() == PlayerCount)
            PlayerCount == 0;

        if (!Player_gay[PlayerCount])
            continue;

        cout << "\n\nХод игрока №" << PlayerCount + 1 << (Player_Ai[PlayerCount] ? ": Ai" : ": Player") <<"\nНазовите город России на букву: ";
        if (Dick == '0') cout << "На любую букву";
        else{
            cout << Dick;
            cout << "\nОсталось городов на букву " << Dick << ": " << CountTownsOnDick(Dick);

        }
        if (Player_Ai[PlayerCount])
            PlayerAiTurn();
        else
            PlayerGayTurn();
    }
